...
pub struct CrsInput<'a> {
    ...
    pub input_type: &'a mut CrsInputType,
...
impl egui::Widget for CrsInput<'_> {
    fn ui(self, ui: &mut egui::Ui) -> egui::Response {
        ui.vertical(|ui| {
            ui.horizontal(|ui| {
                ui.radio_value(self.input_type, CrsInputType::Epsg, "EPSG");
                ui.radio_value(self.input_type, CrsInputType::Proj, "Proj");
            });

            ui.add(InputFieldWidget::new(
                self.geodesy_ctx,
...
struct InputFieldWidget<'a> {
    ...
    text_field_value: &'a mut String,
    outcome: &'a mut Option<Outcome>,
    input_type: &'a CrsInputType,
}

impl<'a> InputFieldWidget<'a> {
    pub fn new(
        geodesy_ctx: &'a rgis_geodesy::GeodesyContext,
        text_field_value: &'a mut String,
        outcome: &'a mut Option<Outcome>,
...
impl egui::Widget for InputFieldWidget<'_> {
    fn ui(self, ui: &mut egui::Ui) -> egui::Response {
        ui.horizontal(|ui| {
            ...
            *self.outcome = if edit_field.changed()
                || (!self.text_field_value.is_empty() && self.outcome.is_none())
            {
                ...
                Some(match self.input_type {
                    CrsInputType::Epsg => {
                        parse_epsg_input_value(self.geodesy_ctx, self.text_field_value)
                    }
                    CrsInputType::Proj => {
                        parse_proj_string_input_value(self.geodesy_ctx, self.text_field_value)
                    }
                })
...
pub enum Error {
    ...
    RgisGeodesy(rgis_geodesy::Error),
}
...
impl std::fmt::Display for Error {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Error::RgisGeodesy(e) => write!(f, "rgis-geodesy error: {}", e),
        }
    }
}
...
impl From<rgis_geodesy::Error> for Error {
    fn from(err: rgis_geodesy::Error) -> Self {
        Error::RgisGeodesy(err)
    }
}

fn parse_proj_string_input_value(
    geodesy_ctx: &rgis_geodesy::GeodesyContext,
    input: &str,
) -> Outcome {
    let mut geodesy_ctx = geodesy_ctx.0.write().unwrap();
    let proj = geodesy::parse_proj(input)?;
    let op_handle = geodesy_ctx.op(&proj)?;
    Ok((op_handle, 0))
}

fn parse_epsg_input_value(geodesy_ctx: &rgis_geodesy::GeodesyContext, input: &str) -> Outcome {
    let mut geodesy_ctx = geodesy_ctx.0.write().unwrap();
    let parsed = u16::from_str(input)?;
    let outcome = rgis_geodesy::epsg_code_to_geodesy_op_handle(&mut *geodesy_ctx, parsed)
        .map_err(Error::RgisGeodesy)?;
    Ok((outcome, parsed))
}
