...
impl WelcomeContent {
    pub fn show(self, ui: &mut egui::Ui) -> (egui::Response, Option<WelcomeAction>) {
        let mut action = None;
        let response = ui
            .vertical_centered_justified(|ui| {
                ...
                if ui.button("Add Layer...").clicked() {
                    action = Some(WelcomeAction::AddLayer);
                }
            })
            .response;
        (response, action)
...
pub struct Welcome<'w> {
    show_add_layer_window_event_writer:
        MessageWriter<'w, rgis_ui_messages::ShowAddLayerWindowMessage>,
}

impl egui::Widget for Welcome<'_> {
    fn ui(mut self, ui: &mut egui::Ui) -> egui::Response {
        let (response, action) = WelcomeContent.show(ui);
        if let Some(action) = action {
            match action {
                WelcomeAction::AddLayer => {
